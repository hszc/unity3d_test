...
dmDeviceName;
        ...
        public static void DeleteCustomPaperSize(string PrinterName, string PaperName)
        {
            ...
            if (OpenPrinter(PrinterName, out hPrinter, ref defaults))
            {
                ...
                {
                    System.Windows.Forms.MessageBox.Show("删除自定义纸张时发生错误！");
                ...
                if (OpenPrinter(PrinterName, out hPrinter, ref defaults))
                {
                    try
                    {
                        ...
                        SendMessageTimeout(
                           ...
                           IntPtr.Zero,
                           IntPtr.Zero,
                           MyPrinter.SendMessageTimeoutFlags.SMTO_NORMAL,
                           1000,
                           out hDummy);
                    }
                    finally
        ...
        public static string GetDeaultPrinterName()
        {
            ...
            {
                int rc = GetLastError();
                System.Windows.Forms.MessageBox.Show("获取默认打印机失败！错误代号：" + rc.ToString());
